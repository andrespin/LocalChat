...
public class MyServer {
    ...
    private final String path = "MyLogFile.log";

    private final ServerLogger serverLogger = new ServerLogger(path);

    public MyServer(int port) throws IOException {
        this.serverSocket = new ServerSocket(port);
        this.authService = new BaseAuthService(this);
    }

    public void start() throws IOException {
        System.out.println("Сервер запущен!");
        serverLogger.writeInfoLog("Сервер запущен!");
        try {
            while (true) {
            ...
            serverLogger.writeSevereLog("Ошибка создания нового подключения");
            e.printStackTrace();
        } finally {
            serverSocket.close();
        }
    ...
    private void waitAndProcessNewClientConnection() throws IOException {
        ...
        serverLogger.writeInfoLog("Ожидание пользователя...");
        Socket clientSocket = serverSocket.accept();
     //        clientSocket.setSoTimeout(120000);
        System.out.println("Клиент подключился!");
        serverLogger.writeInfoLog("Клиент подключился!");
